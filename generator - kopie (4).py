...
if "Kategorie" in df.columns and not pd.isna(row["Kategorie"]):
    ...
    if kategorie_root is not None:
        ...
        if cilova is not None:
            ...
            # Zviditelni i vsechny jeji podskupiny (vnoøené <g>)
    ...
    if "Vzacnost" in df.columns and not pd.isna(row["Vzacnost"]):
        ...
        vzacnost_root = (
            root.find(".//svg:g[@id='Vzacnost']", namespaces)
            or root.find(".//svg:g[@inkscape:label='Vzacnost']", namespaces)
        )

        if vzacnost_root is not None:
