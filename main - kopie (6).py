...
class ScriptGUI:
    ...
    def run_script(self, name): # sourcery skip: raise-specific-error
        ...
        self._write_console(f"[{datetime.now().strftime('%H:%M:%S')}] Nástroj '{name.capitalize()}' spuštěn", bold_time=True)
    ...
    def _execute_script(self, name):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                self._write_console(f"\t Nástroj '{name.capitalize()}' skončil s chybou (exit code: {process.returncode}).")
        except Exception as e:
            self._write_console(f"\t Chyba při spuštění nástroje: {e}")
        finally:
            self.is_script_running = False
            ...
            if script_successful:
                self._write_console(f"Nástroj '{name.capitalize()}' dokončen, běžel {elapsed:.2f} s")
