...
def svg_to_png_bytes(svg_path, dpi=150):
    """Převede SVG na PNG přes Inkscape CLI do paměti."""
...
def parse_svg_length(value):
    """Odstraní jednotky z SVG délky a vrátí float."""
    if value is None:
        return 0.0
...
class SVGEditor(tk.Tk):
    ...
    def load_svg(self, index):
        ...
        if self.original_img:
            self.tk_img = ImageTk.PhotoImage(self.original_img.resize(
    ...
    # --- Uložení SVG s relativními souřadnicemi a odstraněním jednotek ---
    def save_svg(self):
        if self.original_img is None:
            messagebox.showerror("Chyba", "Nejdříve vložte obrázek")
            return
        try:
            # Převod pozice a velikosti z canvasu na SVG souřadnice
            ...
            rel_x = self.image_pos[0] / self.img.width * svg_width
            rel_y = self.image_pos[1] / self.img.height * svg_height
            rel_w = self.image_size[0] / self.img.width * svg_width
            rel_h = self.image_size[1] / self.img.height * svg_height

            # Použití TemporaryDirectory (Windows safe)
