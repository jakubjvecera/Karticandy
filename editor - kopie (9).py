...
def svg_to_png_bytes(svg_path, dpi=150):
    ...
    with tempfile.NamedTemporaryFile(suffix=".png") as tmp:
        ...
        if result.returncode != 0:
            raise RuntimeError(f"Inkscape selhalo:\n{result.stdout}\n{result.stderr}")

        if os.path.getsize(tmp.name) == 0:
            raise RuntimeError(f"PNG nebyl vytvořen, tmp soubor je prázdný: {tmp.name}")
    ...
    def load_svg(self, index):
        ...
        def load_image_thread():
            try:
                ...
                # aktualizace canvasu musí proběhnout v hlavním vlákně
                self.after(0, self.center_display_svg)

                # parsování SVG s huge_tree
                parser = ET.XMLParser(huge_tree=True)
                self.tree = ET.parse(self.current_svg_path, parser)
                ...
                self.after(0, lambda e=e: messagebox.showerror("Chyba", f"Nepodařilo se načíst SVG:\n{e}"))

        threading.Thread(target=load_image_thread, daemon=True).start()

    def preload_svg_cache(self):
        indices_to_preload = [(i % len(self.svg_files)) for i in range(self.current_index - 3, self.current_index + 6)]
