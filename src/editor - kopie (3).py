...
SAVED_FILES_JSON = DATA_FOLDER / "saved_files.json"
...
class SVGEditor(TkinterDnD.Tk):
    def __init__(self):
        ...
        if SAVED_FILES_JSON.exists():
            try:
                with open(SAVED_FILES_JSON, "r", encoding="utf-8") as f:
                    paths = json.load(f)
                    self.saved_files = set(Path(p) for p in paths)
            except Exception:
    ...
    def setup_ui(self):
        ...
        self.tree.tag_configure("saved_file", foreground="green")  # zelená barva

        # --- Label pro počet uložených ---
        ...
        for cat, files in sorted(categories.items()):
            ...
            for file, full_path in sorted(files):
                ...
                if full_path in self.saved_files:
                    tags = ("saved_file",)
                self.tree.insert(cat_id, "end", text=file, values=[str(full_path)], tags=tags)
    ...
    def save_svg(self):
        ...
        try:
            ...
            # --- přidání do seznamu uložených a uložení na disk ---
            self.saved_files.add(self.current_svg_path)
            with open(SAVED_FILES_JSON, "w", encoding="utf-8") as f:
