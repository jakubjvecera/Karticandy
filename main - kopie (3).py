...
# ---------------- Constants ----------------
PROJECTS_DIR = Path("projekt")
SRC_DIR = Path("src")

# Ensure base directories exist
for path in [SRC_DIR, PROJECTS_DIR]:
...
def select_project_dialog(root):
...
class ScriptGUI:
    ...
    def _execute_script(self, name):
        ...
        try:
            ...
            self._write_console(f"[{datetime.now().strftime('%H:%M:%S')}] {name.capitalize()} dokončen, běžel {elapsed:.2f} s", bold_time=True)
            self._write_console("")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()
    project_name = select_project_dialog(root)
    if not project_name:
